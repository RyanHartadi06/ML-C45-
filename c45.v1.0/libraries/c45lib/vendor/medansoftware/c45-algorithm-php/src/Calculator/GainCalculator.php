...
class GainCalculator extends AbstractCalculator
{
	...
	public function calculateGainOfAttribute($attributeName, $criteria = [])
	{
		...
		$gain = $this->gain($this->targetCount, $attributeCount,$attributeName);

		return $gain;
	}

	private function gain($classifier_values, $values,$attributeName)
	{
		$entropy_all = $this->entropy($classifier_values,$attributeName);
		...
		foreach ($values as $sub_values)
		{
			try
			{
				...
				$entropy = $this->entropy($sub_values,$attributeName);
	...
	private function entropy(array $values,$attributeName)
	{
		$result = 0;
